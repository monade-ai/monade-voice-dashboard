...
export function FilterBar({ filters, onFilterChange }: FilterBarProps) {
    ...
    const activeCount = filters.verdicts.length + filters.qualities.length + (filters.hasConversation ? 1 : 0) + (filters.timeRange !== 'all' ? 1 : 0);

    return (
        <div className="flex flex-col gap-3 w-full">
            {/* Unified Glass Island */}
            <div className="flex items-center bg-muted/10 backdrop-blur-xl border border-border/10 rounded-full h-9 px-1.5 gap-1 shadow-sm overflow-hidden">
                {/* Seamless Search */}
                <div className="relative flex-1 min-w-[140px] group h-full flex items-center">
                    ...
                    <Input
                        ...
                        className="pl-9 bg-transparent border-none focus-visible:ring-0 h-full text-[12px] placeholder:text-muted-foreground/30 shadow-none ring-offset-0"
                    />
                </div>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className={cn(
                            ...
                            filters.verdicts.length > 0 ? "text-foreground" : "text-muted-foreground/60"
                        )}>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className={cn(
                            "h-7 gap-1.5 hover:bg-white/[0.05] rounded-full text-[10px] font-bold uppercase tracking-wider px-3 transition-all",
                            filters.qualities.length > 0 ? "text-foreground" : "text-muted-foreground/60"
                        )}>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className={cn(
                            "h-7 gap-1.5 hover:bg-white/[0.05] rounded-full text-[10px] font-bold uppercase tracking-wider px-3 transition-all",
                            filters.timeRange !== 'all' ? "text-foreground" : "text-muted-foreground/60"
                        )}>
                ...
                <Button
                    ...
                    className={cn(
                        "h-7 gap-1.5 hover:bg-white/[0.05] rounded-full text-[10px] font-bold uppercase tracking-wider px-3 transition-all",
                        filters.hasConversation ? "text-primary hover:text-primary hover:bg-primary/5" : "text-muted-foreground/60"
                    )}
                >
                    <Activity className="w-3 h-3 transition-colors" />
                    Engaged
