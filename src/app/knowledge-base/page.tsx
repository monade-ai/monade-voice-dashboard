...
const getIntelligenceInsights = (text: string) => {
  ...
  return { tokens: estTokens, language };
};
        ...
        <div className="mt-6 pt-4 border-t border-border/40 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1"><Cpu size={10} className="text-primary" /><span className="text-[10px] font-mono font-bold text-foreground">~{insights.tokens}</span></div>
            </div>
            <div className="flex flex-col">
  ...
  <div onClick={() => onEdit(item)} className="group flex items-center justify-between p-4 border-b border-border/10 hover:bg-muted/30 transition-all cursor-pointer">
    <div className="flex items-center gap-4 flex-1">
      ...
      <div className="flex flex-col">
        ...
        <span className="text-[10px] text-muted-foreground uppercase tracking-widest">Added {item.createdAt.toLocaleDateString()}</span>
      </div>
    </div>
...
export default function LibraryPage() {
  ...
  const getConnectedAssistants = useCallback((kbId: string) => assistants.filter(a => a.knowledgeBase === kbId), [assistants]);
      ...
      <main className="flex-1 p-6 md:p-8 max-w-7xl mx-auto w-full space-y-12">
        ...
        <div className="flex items-end justify-between border-b border-border/40 pb-8">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm max-w-sm leading-relaxed uppercase tracking-[0.1em] font-bold">Memory Fragments</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground/60" />
              <Input placeholder="Recall memory..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-10 w-64 bg-muted/10 border-border/40 text-xs focus:ring-primary focus:border-primary transition-all rounded-md" />
            </div>
            <Button onClick={() => setIsWorkshopOpen(true)} className="h-10 px-4 gap-2 bg-foreground text-background hover:bg-foreground/90 transition-all rounded-[4px] text-[10px] font-bold uppercase tracking-[0.2em]"><Plus size={16} />Add Intelligence</Button>
          </div>
        </div>
        ...
        {isLoading && items.length === 0 ? (
          ...
          <div className="space-y-20">
            ...
            {/* 1. Active Intelligence (Pinned Cards) */}
            <section className="space-y-6">
              <div className="flex items-center gap-4">
                <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-foreground">Active Intelligence</h2>
                <div className="h-px flex-1 bg-border/20" />
                <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20 text-[9px] font-mono">{linkedItems.length} ONLINE</Badge>
              </div>
              {linkedItems.length === 0 ? (
                <div className="py-12 text-center border border-dashed border-border/40 rounded-lg bg-muted/5"><p className="text-xs text-muted-foreground italic tracking-widest">No memories inhabit your assistants yet</p></div>
              ) : (
            ...
            {/* 2. Archive Ledger (Filtered & Paginated) */}
            <section className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/40 pb-4">
                <div className="flex items-center gap-4">
                  <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">Archive Ledger</h2>
                  <div className="flex p-1 bg-muted rounded-md">
                    {['all', '7d', '30d'].map((f) => (
                      ...
                      >
                        {f === 'all' ? 'All Time' : f === '7d' ? 'Past Week' : 'Past Month'}
                      </button>
                    ))}
                  </div>
                </div>
                <div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-widest text-foreground/70"><CalendarDays size={12} className="text-primary/60" /><span>{filteredArchive.length} Fragments Found</span></div>
              </div>

              <div className="bg-card/30 rounded-md border border-border/20 overflow-hidden min-h-[400px] flex flex-col">
                <div className="flex-1">
                  {paginatedArchive.length === 0 ? (
                    <div className="p-20 text-center text-xs text-muted-foreground italic uppercase tracking-widest">No standalone fragments found in this range</div>
                  ) : (
                    <div className="flex flex-col">
